...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		new_state.header.stamp = ros::Time::now();
		new_state.position[0] = M_PI* cos(diff/2.);// Complete this
		new_state.position[1] = M_PI * sin(diff/2.);// Complete this
		new_state.position[2] = M_PI*sin(diff/3.);// Complete this
