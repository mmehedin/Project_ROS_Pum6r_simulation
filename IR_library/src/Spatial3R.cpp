...
namespace IRlibrary
{
    void Spatial3R::zeroForwardKinematics() {
        ...
        v2.resize(q.size());
        Vec3::Map(&v2[0], q.size()) = q;
        SE3Mat T = FKinSpace(M, Slist, v2);
        Vec3 p;
        SO3Mat R;
        TransToRp(T, R, p);
    ...
    bool Spatial3R::inverseKinematics_numerical(Eigen::VectorXd const &initial_thetaList, Vec3 const & x_in){
        SE3Mat T;
...
//        Vec2 xy_i = {};
	...
	bool Spatial3R::checkInDexWs(Vec3 x_in){
